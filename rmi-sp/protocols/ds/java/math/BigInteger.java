 ...
 * descriptions of BigInteger methods. The pseudo-code expression {@code (i +
 * j)} is shorthand for "a BigInteger whose value is that of the BigInteger
 * {@code i} plus that of the BigInteger {@code j}." The pseudo-code expression
 * {@code (i == j)} is shorthand for "{@code true} if and only if the BigInteger
 * {@code i} represents the same value as the BigInteger {@code j}." Other
...
public class BigInteger extends Number {
	 ...
	 * magnitude must be "minimal" in that the most-significant int ({@code
	 * mag[0]}) must be non-zero. This is necessary to ensure that there is
	 * exactly one representation for each BigInteger value. Note that this
	 ...
	 * optional minus followed by a sequence of one or more digits in the
	 ...
	 * the range {@code 0} to (2<sup>{@code numBits}</sup> - 1), inclusive. The
	...
	/* taken from OpenJDK sources */
	static private void nextBytes(Random random, byte[] bytes) {
		for (int i = 0, len = bytes.length; i < len;)
			for (int rnd = random.nextInt(), n = Math.min(len - i, 8
					/ 1); n-- > 0; rnd >>= 1)
				bytes[i++] = (byte) rnd;
	}
	 ...
	 *            prime number will exceed (1 - 1/2<sup>{@code certainty}
	 *            </sup>). The execution time of this constructor is
	 ...
	 *            that this BigInteger is prime exceeds {@code (1 -
	 *            1/2<sup>certainty</sup>)}. The execution time of this method
	...
	private static final int[] squareToLen(int[] x, int len, int[] z) {
		 ...
		 * "abcde" by itself:
		 * 
		 * a b c d e a b c d e ================== ae be ce de ee ad bd cd dd de
		 * ac bc cc cd ce ab bb bc bd be aa ab ac ad ae
		 * 
		 * Note that everything above the main diagonal: ae be ce de = (abcd) e
		 * ad bd cd = (abc) d ac bc = (ab) c ab = (a) b
		 * 
	 ...
	 * {@code abs(this)} and {@code abs(val)}. Returns 0 if {@code this==0 &&
	 * val==0}.
	 * 
	 * @param val
	...
	public BigInteger modPow(BigInteger exponent, BigInteger m) {
		...
		BigInteger base = (this.signum < 0 || this.compareTo(m) >= 0 ? this
				.mod(m) : this);
		BigInteger result;
		if (m.testBit(0)) { // odd modulus
			...
			BigInteger base2 = (this.signum < 0 || this.compareTo(m1) >= 0 ? this
					.mod(m1)
					: this);

			// Caculate (base ** exponent) mod m1.
			BigInteger a1 = (m1.equals(ONE) ? ZERO : base2.oddModPow(exponent,
					m1));
	...
	private BigInteger oddModPow(BigInteger y, BigInteger z) {
		...
		for (int myi = 0; myi < mag.length; myi++) {
			base[myi] = mag[myi];
		}
		// int[] base = mag.clone();
		int[] exp = y.mag;
	...
	/**
	 * Returns a BigInteger whose value is {@code (this}<sup>-1</sup> {@code mod
	 * m)}.
	 * 
	 * @param m
	 *            the modulus.
	 ...
	 * @throws ArithmeticException
	 *             {@code m <= 0}, or this BigInteger has no multiplicative
	...
	public BigInteger and(BigInteger val) {
		...
		for (int i = 0; i < result.length; i++)
			result[i] = (getInt(result.length - i - 1) & val
	 ...
	 * binary representation. (Computes {@code (ceil(log2(this < 0 ? -this :
	 * this+1)))}.)
	 * 
	...
	/**
	 * Returns {@code true} if this BigInteger is probably prime, {@code false}
	 * if it's definitely composite. If {@code certainty} is {@code <= 0},
	 * {@code true} is returned.
	 * 
	 * @param certainty
	 *            a measure of the uncertainty that the caller is willing to
	 *            tolerate: if the call returns {@code true} the probability
	 *            that this BigInteger is prime exceeds (1 - 1/2<sup>{@code
	 *            certainty}</sup>). The execution time of this method is
	 *            proportional to the value of this parameter.
	 * @return {@code true} if this BigInteger is probably prime, {@code false}
	 *         if it's definitely composite.
	 */
	...
	public int compareTo(BigInteger val) {
		return (signum == val.signum ? signum * intArrayCmp(mag, val.mag)
	...
	public String toString(int radix) {
		...
		while (tmp.signum != 0) {
			...
			MutableBigInteger q = new MutableBigInteger(), r = new MutableBigInteger(), a = new MutableBigInteger(
					tmp.mag), b = new MutableBigInteger(d.mag);
			a.divide(b, q, r);
		...
		// StringBuilder(numGroups*digitsPerLong[radix]+1);
		String buf = new String("" + numGroups * digitsPerLong[radix] + 1);
		if (signum < 0)
			buf += ("-");
	 ...
	 * BigInteger, including at least one sign bit, which is {@code
	 * (ceil((this.bitLength() + 1)/8))}. (This representation is compatible
	 * with the {@link #BigInteger(byte[]) (byte[])} constructor.)
	 * 
	 ...
	 * to a <a href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"
	 * ><i>narrowing primitive conversion</i></a> from {@code long} to {@code
	 * int} as defined in the <a
	 * href="http://java.sun.com/docs/books/jls/html/">Java Language
	 * Specification</a>: if this BigInteger is too big to fit in an {@code int}
	 * , only the low-order 32 bits are returned. Note that this conversion can
	 * lose information about the overall magnitude of the BigInteger value as
	 ...
	 * int} as defined in the <a
	 * href="http://java.sun.com/docs/books/jls/html/">Java Language
	 * Specification</a>: if this BigInteger is too big to fit in a {@code long}
	 ...
	 * to the <a href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"
	 * ><i>narrowing primitive conversion</i></a> from {@code double} to {@code
	 * float} defined in the <a
	 * href="http://java.sun.com/docs/books/jls/html/">Java Language
	 ...
	 * integer n such that radixn < 263. The second is the "long radix" that
	 ...
	 * (longRadix[i] = idigitPerLong[i]). Both arrays have nonsense values in
	...
	private byte[] magSerializedForm() {
		int bitLen = (mag.length == 0 ? 0 : ((mag.length - 1) << 5)
				+ bitLen(mag[0]));
